...
using System.IO;
using System.Text;
using POS.Internals.I18N.Loaders;

namespace POS.Internals.I18N
{
    /// <summary>
    ...
    public class Catalog : BaseCatalog
    {
        ...
        /// </summary>
        public Catalog() : base(CultureInfo.CurrentUICulture)
        {
        }
        ...
        public Catalog(CultureInfo cultureInfo) : base(cultureInfo)
        {
        }
        ...
        public Catalog(Stream moStream) : this()
        {
            try
            {
                this.Load(moStream);
        ...
        /// <param name="moStream">Stream that contain binary data in the MO file format</param>
        /// <param name="cultureInfo">Culture info</param>
        public Catalog(Stream moStream, CultureInfo cultureInfo) : this(cultureInfo)
        {
            try
            {
                this.Load(moStream);
        ...
        public Catalog(string domain, string localeDir) : this()
        {
            try
            {
        ...
        /// <param name="localeDir">Directory that contains gettext localization files</param>
        /// <param name="cultureInfo">Culture info</param>
        public Catalog(string domain, string localeDir, CultureInfo cultureInfo) : this(cultureInfo)
        {
            try
            {
                this.Load(this.CultureInfo, domain, localeDir);
        ...
        {
            var relativePath =
                string.Format("{0}{1}LC_MESSAGES{2}{3}.mo", locale.Replace('-', '_'), Path.DirectorySeparatorChar, Path.DirectorySeparatorChar, domain);
