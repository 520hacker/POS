...
namespace POS.Internals.FilterBuilder
{
    ...
    public class FilterGroup
    {
        ...
        /// </summary>
        private readonly List<string> sFilters;
        ...
        public void Add(POS.Internals.FilterBuilder.FilterBuilder.Filters filter)
        {
        ...
        public void Add(POS.Internals.FilterBuilder.FilterBuilder.Filters[] filters)
        {
            //Add filter to the list
            foreach (POS.Internals.FilterBuilder.FilterBuilder.Filters item in filters)
            {
        ...
        public void Add(string extension, bool plural = true)
        {
            ...
            try
            {
                description = new POS.Internals.FilterBuilder.FilterBuilder().GetSystemFileTypeDescription(extension);
            }
            catch (Exception)
            {
                description = "";
        ...
        public string GetFilterString()
        {
            ...
            foreach (string s in this.sFilters)
            {
            ...
            string fs = string.Format("{0} (", this.GroupName.Trim(' ', '*', '.'));
            ...
            for (int i = 0; i <= ext.Count - 1; i++)
            {
                ...
                if (i != ext.Count - 1)
                {
                    ...
                    if (!ext[i].StartsWith("*."))
                    {
                        fs += string.Format("*.{0},", ext[i]).Replace(";", ",");
                    }
                    else
                    {
                        fs += string.Format("{0},", ext[i]).Replace(";", ",");
            ...
            for (int i2 = 0; i2 <= ext.Count - 1; i2++)
            {
                ...
                if (i2 != ext.Count - 1)
                {
                    ...
                    {
                        fs += string.Format("*.{0};", ext[i2]).Replace(",", ";");
